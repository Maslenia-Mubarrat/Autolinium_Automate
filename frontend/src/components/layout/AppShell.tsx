...
export function AppShell({ children }: { children: React.ReactNode }) {
    ...
    return (
        <SidebarProvider>
            <div className="flex min-h-screen w-full bg-white font-mono">
                {/* Only show the Sidebar if we are on Desktop */}
                {!isMobile && <AppSidebar />}

                <main className="flex-1 flex flex-col min-w-0">
                    {/* A professional top header shared by everyone */}
                    <header className="h-14 border-b-2 border-primary flex items-center px-4 justify-between bg-white sticky top-0 z-40">
                        ...
                        <div className="flex items-center gap-2 px-2 py-1 bg-slate-50 border border-slate-200">
                            <div className="h-1.5 w-1.5 rounded-full bg-green-500 animate-pulse" />
                            <span className="text-[10px] uppercase font-bold text-slate-500 font-mono tracking-tighter">
                                Sync_Live
                            </span>
                        </div>
                    </header>
